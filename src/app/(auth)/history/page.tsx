      ...
      <div className="w-80 flex-shrink-0 flex flex-col gap-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  {!editMode && (
                    <>
                      ...
                      <button onClick={startEdit} className="flex items-center gap-1 px-2.5 py-1.5 text-gray-400 border border-gray-600 rounded-lg text-xs hover:bg-gray-700">
                        <Edit3 size={11} />수정
                      </button>
                      <button onClick={() => deletePost(selectedPost.id)} className="flex items-center gap-1 px-2.5 py-1.5 text-red-400 border border-red-800 rounded-lg text-xs hover:bg-red-900/30">
                        <Trash2 size={11} />삭제
                      </button>
                    </>
                  )}
...
function StatusDot({ done, label }: { done: boolean; label: string }) {
  return (
    <span className={`text-xs px-1.5 py-0.5 rounded-full ${done ? 'bg-green-900/50 text-green-400' : 'bg-gray-700 text-gray-500'}`}>
      {label}
    </span>
  )
}
